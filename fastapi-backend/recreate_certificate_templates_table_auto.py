"""
Script to drop and recreate certificate_templates table (non-interactive)
    ...
    try:
        ...
        async with engine.begin() as conn:
            ...
            try:
                ...
                for table, column in fk_constraints:
                    try:
                        ...
                        if result.scalar() > 0:
                            ...
                            """)
                            col_result = await conn.execute(check_column)
                            if col_result.scalar() > 0:
                                ...
                                check_fk = text(f"""
                                    ...
                                    AND REFERENCED_TABLE_NAME = 'certificate_templates'
                                """)
                                fk_result = await conn.execute(check_fk)
                                if fk_result.scalar() == 0:
                ...
                print("  Foreign keys can be recreated later if needed")
