Here’s your **updated `README.md`** with MySQL instead of MongoDB 👇

---

```markdown
# FastAPI Backend - infodocs

FastAPI backend for certificate and document management system using **MySQL**.

## Project Structure

```

fastapi-backend/
├── main.py                 # FastAPI application entry point
├── config/                 # Configuration
│   ├── settings.py         # Application settings
│   └── database.py         # MySQL connection (SQLAlchemy)
├── core/                   # Core utilities
│   ├── exceptions.py       # Custom exceptions
│   ├── middleware.py       # Custom middleware
│   ├── dependencies.py     # FastAPI dependencies
│   └── utils.py            # Utility functions
├── apps/                   # Application modules
│   ├── users/              # User management
│   │   ├── models.py       # SQLAlchemy models
│   │   ├── schemas.py      # Pydantic schemas
│   │   ├── services/       # Business logic
│   │   └── routers.py      # API routes
│   ├── certificates/       # Certificate management
│   ├── forms_app/          # Form submissions
│   ├── analytics/          # Analytics
│   └── notifications/      # Notifications
├── requirements.txt        # Python dependencies
└── .env.example            # Environment variables example

````

---

## Setup

### 1. Create virtual environment
```bash
python -m venv venv
source venv/bin/activate  # On Windows: venv\Scripts\activate
````

---

### 2. Install dependencies

```bash
pip install -r requirements.txt
```

---

### 3. Configure environment

```bash
cp .env.example .env
```

Update `.env` with your MySQL credentials:

```env
DATABASE_URL=mysql+pymysql://root:password@localhost:3306/infodocs
```

---

### 4. Setup MySQL Database

Make sure MySQL is installed and running.

Create database manually:

```sql
CREATE DATABASE infodocs;
```

---

### 5. Run the server

```bash
python main.py
```

or

```bash
uvicorn main:app --reload --host 0.0.0.0 --port 8000
```

---

## API Documentation

Once the server is running, visit:

* Swagger UI: [http://localhost:8009/docs](http://localhost:8009/docs)
* ReDoc: [http://localhost:8009/redoc](http://localhost:8009/redoc)

---

## Environment Variables

Key environment variables (see `.env.example`):

* `DATABASE_URL`: MySQL connection string
* `SECRET_KEY`: Secret key for JWT tokens
* `DEBUG`: Enable debug mode (default: `True`)
* `CORS_ORIGINS`: Allowed CORS origins (comma-separated)

---

## Features

* ✅ User authentication (JWT tokens)
* ✅ User registration and login
* ✅ OTP-based authentication
* ✅ Role-based access control (Admin, HR, Manager, Candidate)
* ✅ MySQL integration (SQLAlchemy ORM)
* ✅ RESTful API structure
* ✅ Automatic API documentation
* ✅ CORS support
* ✅ Error handling

---

## API Endpoints

### Authentication

* `POST /api/users/auth/register` - User registration
* `POST /api/users/auth/login` - Login with username/email
* `POST /api/users/auth/login_with_email` - Login with email
* `POST /api/users/auth/request_login_otp` - Request OTP
* `POST /api/users/auth/login_with_otp` - Login with OTP
* `GET /api/users/auth/user` - Get current user

---

### Users

* `GET /api/users` - List users (Admin/HR only)
* `GET /api/users/{user_id}` - Get user by ID
* `PUT /api/users/{user_id}` - Update user

---

### Certificates

* `GET /api/certificates/templates` - List templates
* `POST /api/certificates/templates` - Create template

---

### Forms

* `GET /api/forms/spas` - List SPAs
* `POST /api/forms/candidate-forms` - Submit form

---

### Analytics

* `GET /api/analytics` - Get analytics (Admin/HR/Manager)

---

## Development

The project follows FastAPI best practices:

* Separation of concerns (models, schemas, services, routers)
* Dependency injection for authentication
* Role-based access control
* SQLAlchemy ORM for database operations
* Pydantic for data validation

---

## Database Notes

* Tables are created using SQLAlchemy models
* Use migrations (Alembic recommended) for schema updates
* Relationships handled via foreign keys
* All timestamps are stored in UTC

---

## Optional (Recommended)

### Use Alembic for migrations

```bash
alembic init alembic
alembic revision --autogenerate -m "initial migration"
alembic upgrade head
```

---

## Notes

* JWT tokens expire after 30 days (configurable)
* OTP codes expire after 10 minutes
* Use indexed fields (e.g., email) for performance
* Ensure proper database connection pooling in production

```

---

If you want, I can also:
- Add **Docker setup (MySQL + FastAPI)**
- Add **production-ready config (Gunicorn + Nginx)**
- Or optimize for **high-scale (connection pooling + async MySQL)**
```
