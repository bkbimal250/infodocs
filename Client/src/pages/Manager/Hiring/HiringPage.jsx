...
/**
 * Manager Hiring Page
 * View manager's own submitted hiring forms
 */
const HiringPage = () => {
  ...
  const loadHiringForms = async () => {
    try {
      ...
      // Get manager's own hiring forms - backend should automatically filter by current_user.id
      const response = await managerApi.getMyHiringForms({
        skip: 0,
        limit: 1000,
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await managerApi.deleteHiringForm(id);
