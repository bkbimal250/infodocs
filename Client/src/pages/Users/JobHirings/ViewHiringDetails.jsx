 ...
 * Display detailed information about a hiring form (User version)
 */
const ViewHiringDetails = () => {
  const { id } = useParams();
  ...
  const loadHiringForm = async () => {
    try {
      setLoading(true);
      const response = await usersApi.getHiringForm(id);
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-8 px-4">
        <div className="max-w-4xl mx-auto">
          <button
            onClick={() => navigate('/user/job-hirings')}
            className="text-blue-600 hover:text-blue-800 mb-4"
          >
            <HiArrowLeft className="inline mr-1" /> Back to My Hiring Forms
          </button>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <tbody className="bg-white divide-y divide-gray-200">
                ...
                <tr className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-900">
                    {hiringForm.spa?.name || hiringForm.spa_name_text || 'N/A'}
                    {hiringForm.spa?.city && hiringForm.spa?.state && (
...
export default ViewHiringDetails;
