 ...
 * Shows all certificates (HR can see all certificates from all users)
 */
const Certificates = () => {
  ...
  const loadCertificates = async () => {
    ...
    try {
      setLoading(true);
      // Get all certificates (HR can see all certificates)
      const response = await hrApi.getCertificates({ skip: 0, limit: 1000 });
      const certificates = response.data?.results || response.data || [];
        ...
        <div className="bg-[var(--color-bg-primary)] rounded-lg shadow overflow-hidden">
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-[var(--color-text-secondary)] mb-6">No certificates found.</p>
              <Link
                to="/hr/certificate-creation"
                className="inline-flex items-center px-4 py-2 bg-[var(--color-primary)] text-white rounded-lg hover:bg-[var(--color-primary-dark)] transition-colors"
              >
